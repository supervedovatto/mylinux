...
echo "Apaga os Downloads anteriores da pasta /tmp*****************************************************************"
VimSource=/tmp/vim
if [ -d "$VimSource"]; then
    rm -rf $VimSource
fi
...
if [ ! -f $Vim/myvim_amd64.deb ]; then
	...
	sudo checkinstall
	cp /tmp/vim/*.deb $Vim/myvim_amd64.deb
else
	echo "Reaproveitando compilação anterior do vim *******************************"
	sudo dpkg -i $Vim/myvim_amd64.deb
fi
...
cd $MyUSER/.vim
git remote add origin https://bitbucket.org/supervedovatto/vim-config
